...
(function () {
  ...
  function renderPantallaRol() {
    ...
    document.getElementById("btn-alumno").addEventListener("click", function () {
      ...
      if (!FB) { TOAST.error("Firebase no está disponible."); return; }
      if (FB.auth.currentUser) { activarRol("alumno"); return; }
      FB.auth.signInAnonymously().catch(function (err) {
        console.error(err);
        TOAST.error("No se pudo ingresar. Revise su conexión.");
      });
      // activarRol se dispara solo vía onAuthStateChanged al completar el login
    });
  ...
  function mostrarLoginModal() {
    ...
    overlay.innerHTML =
      '<div class="pin-modal">' +
        ...
        '<p>Ingrese su correo y contraseña registrados en Firebase.</p>' +
        '<input type="email" class="pin-input" id="login-email" placeholder="correo@ejemplo.com" autocomplete="username" style="text-align:left;letter-spacing:normal;font-size:1rem;margin-bottom:.6rem"/>' +
        '<input type="password" class="pin-input" id="login-password" placeholder="Contraseña" autocomplete="current-password"/>' +
        '<div class="pin-error" id="pin-error">Correo o contraseña incorrectos.</div>' +
    ...
    var inpEmail = document.getElementById("login-email");
    var inpPass = document.getElementById("login-password");
    var err = document.getElementById("pin-error");
    var btnOk = document.getElementById("btn-pin-ok");

    function verificarLogin() {
      if (!FB) { TOAST.error("Firebase no está disponible."); return; }
      btnOk.disabled = true;
      FB.auth.signInWithEmailAndPassword(inpEmail.value.trim(), inpPass.value)
        .then(function () {
          if (overlay.parentNode) document.body.removeChild(overlay);
          // activarRol("maestro") se dispara vía onAuthStateChanged
        })
        .catch(function () {
    ...
    btnOk.addEventListener("click", verificarLogin);
    document.getElementById("btn-pin-cancelar").addEventListener("click", function () {
      document.body.removeChild(overlay);
    });
    inpPass.addEventListener("keydown", function (e) { if (e.key === "Enter") verificarLogin(); });
    setTimeout(function () { inpEmail.focus(); }, 100);
  }

  function activarRol(rol) {
    if (estado.rol === rol) return; // evita re-render duplicado desde onAuthStateChanged
    estado.rol = rol;
  ...
  function init() {
    ...
    window.UCENM_APP = {
      ...
      refrescar: refrescarSeccionActiva,
      navegarA: navegarA,
    ...
    var FB = window.UCENM_FIREBASE;
    if (!FB) { renderPantallaRol(); return; }
